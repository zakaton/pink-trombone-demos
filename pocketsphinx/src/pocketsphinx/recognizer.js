...
function startup(onMessage) {
  self.onmessage = function (event) {
    var pocketsphinxJS =
      event.data && "pocketsphinx.js" in event.data
        ? event.data["pocketsphinx.js"]
        : "pocketsphinx.js";
    var pocketsphinxWASM =
      event.data && "pocketsphinx.wasm" in event.data
        ? event.data["pocketsphinx.wasm"]
        : "/webapp/js/pocketsphinx.wasm";
    // Case of compilation to WebAssembly, this is an absolute path
    Module["locateFile"] = function () {
      return pocketsphinxWASM;
    };
...
function initialize(data, clbId) {
  ...
  if (recognizer) {
    ...
    if (output != Module.ReturnType.SUCCESS)
      post({ status: "error", command: "initialize", code: output });
...
function lazyLoad(data, clbId) {
  ...
  data["folders"].forEach(function (folder) {
    folders.push([folder[0], folder[1]]);
  });
  data["files"].forEach(function (file) {
    files.push([file[0], file[1], file[2]]);
  });
...
function addGrammar(data, clbId) {
  ...
  if (recognizer) {
    if (
      data.hasOwnProperty("numStates") &&
      data.numStates > 0 &&
      data.hasOwnProperty("start") &&
      data.hasOwnProperty("end") &&
      data.hasOwnProperty("transitions") &&
      ...
      output = recognizer.addGrammar(id_v, {
        start: data.start,
        end: data.end,
...
function lookupWord(data, clbId) {
  if (recognizer) {
    ...
    post({ status: "error", command: "lookupWord", code: "js-no-recognizer" });
}
...
function lookupWords(data, clbId) {
  if (recognizer) {
    ...
    data.forEach(function (word) {
      ...
      if (wid && output.indexOf(word) == -1) output.push(word);
    });
    post({ id: clbId, data: output, status: "done", command: "lookupWords" });
  } else
    post({ status: "error", command: "lookupWords", code: "js-no-recognizer" });
}
...
function process(array) {
  if (recognizer) {
    while (buffer.size() < array.length) buffer.push_back(0);
    for (var i = 0; i < array.length; i++) buffer.set(i, array[i]);
