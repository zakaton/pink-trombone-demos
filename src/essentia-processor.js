...
exports.essentiaExtractor = new EssentiaExtractor(exports.EssentiaWASM);

exports.Float32Concat = function (first, second) {
...
class EssentiaProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this._extractor = exports.essentiaExtractor;
    ...
    this._inputRingBuffer = new exports.ChromeLabsRingBuffer(
      this._bufferSize,
      this._channelCount
    );
    this._outputRingBuffer = new exports.ChromeLabsRingBuffer(
      this._bufferSize,
      this._channelCount
    ); // changed from 1024 to match block size
    ...
    this.port.onmessage = (e) => {
      this._audio_writer = new AudioWriter(
  ...
  process(inputList, outputList, params) {
    ...
    if (this._inputRingBuffer.framesAvailable >= this._bufferSize) {
      ...
      if (this._accumData[0][0] == 0) {
        return true;
      }

      this._spectrum = this._extractor.melSpectrumExtractor(
        this._accumData[0],
        this._sampleRate
      );
      ...
      console.log(pitchFrames, confidenceFrames);
...
exports.BYTES_PER_UNIT = Uint16Array.BYTES_PER_ELEMENT;

// Byte per audio sample. (32 bit float)
exports.BYTES_PER_SAMPLE = Float32Array.BYTES_PER_ELEMENT;

// The max audio channel on Chrome is 32.
exports.MAX_CHANNEL_COUNT = 32;

// WebAudio's render quantum size.
exports.RENDER_QUANTUM_FRAMES = 128;
 ...
 * @class
 */
exports.ChromeLabsRingBuffer = class ChromeLabsRingBuffer {
  /**
   * @constructor
