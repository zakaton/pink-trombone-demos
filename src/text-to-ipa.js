...
(function () {
  ...
  function IPAWord(error, text, ipas) {
    this.error = error;
    this.text = text;
    this.ipas = ipas;
  }
  ...
  if (typeof TextToIPA._parseDict !== "function") {
    TextToIPA._parseDict = function (lines) {
      ...
      for (var i in lines) {
        ...
        let word = arr[0].split("(")[0];
        let pronunciation = arr[1];

        if (word && pronunciation) {
          if (!Array.isArray(TextToIPA._IPADict[word])) {
          ...
          if (!(pronunciation in TextToIPA._WordDict)) {
            TextToIPA._WordDict[pronunciation] = [];
          }
          TextToIPA._WordDict[pronunciation].push(word);
        }
      }
        ...
        if (typeof TextToIPA._IPADict[word] != "undefined") {
          ...
          for (var i = 1; i < 4; i++) {
            ...
            if (
              ...
              ipas.push(TextToIPA._IPADict[word + "(" + i + ")"]);
          ...
          // Return the new word
          return new IPAWord(error, text, ipas);
