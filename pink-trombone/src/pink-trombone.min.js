...
if (window.AudioContext.prototype.createConstantSource == undefined) {
  window.AudioContext.prototype.createConstantSource = function () {
    ...
    constantSourceNode.minValue = -3.402820018375656e38;
    constantSourceNode.maxValue = +3.402820018375656e38;
...
Object.defineProperties(SimplexNoise.prototype, {
  seed: {
    value: function (seed) {
      ...
      for (let index = 0; index < Math.pow(2, 8); index++) {
        const seedShift = index & 1 ? 0 : Math.pow(2, 3);
  ...
  simplex2: {
    value: function (xin, yin) {
      ...
      const n0 = t0 < 0 ? 0 : Math.pow(t0, 4) * gi0.dot2(x0, y0);

      const t1 = 0.5 - Math.pow(x1, 2) - Math.pow(y1, 2);
      const n1 = t1 < 0 ? 0 : Math.pow(t1, 4) * gi1.dot2(x1, y1);

      const t2 = 0.5 - Math.pow(x2, 2) - Math.pow(y2, 2);
      const n2 = t2 < 0 ? 0 : Math.pow(t2, 4) * gi2.dot2(x2, y2);
...
class Glottis {
  ...
  _getNormalizedWaveform(interpolation) {
    return interpolation > this.coefficients.Te
      ? (-Math.exp(-this.coefficients.epsilon * (interpolation - this.coefficients.Te)) + this.coefficients.shift) /
          this.coefficients.Delta
...
class Nose {
  ...
  _onTractUpdate(tract) {
    ...
    for (let index = 0; index < this.length; index++) {
      ...
      const value = interpolation < 0.5 ? 0.4 + 1.6 * (2 * interpolation) : 0.5 + 1.5 * (2 - 2 * interpolation);
...
class PinkTrombone {
  addModules(audioContext) {
    if (audioContext.audioWorklet !== undefined) {
      ...
      return audioContext.audioWorklet.addModule("/pink-trombone/src/pink-trombone-worklet-processor.min.js");
    } else {
      return new Promise((resolve, reject) => {
        resolve();
      });
...
class GlottisUI {
  constructor() {
    ...
    this._container.addEventListener("touchmove", (event) => {
      ...
      if (touch !== undefined) this._eventCallback(touch);
    });
    this._container.addEventListener("touchend", (event) => {
      ...
      if (
        this._isActive &&
        Array.from(event.changedTouches).some((touch) => touch.identifier == this._touchIdentifier)
      ) {
        this._isActive = false;
  ...
  _createButton(buttonName, isParameter = false, parameterPath) {
    ...
    if (isParameter) {
      button.addEventListener("click", (event) => {
        ...
        const prefix = button.value == "true" ? "disable" : "enable";
